...
def contrastive_class_to_class_learned_memory(model, features, class_labels, num_classes, memory):
    ...
    for c in range(num_classes):
        ...
        mask_c = (class_labels == c)  # mask_c.sum()=M pixels
        features_c = features[mask_c, :]  # (M pixels, 256)
        memory_c = memory[c]  # (N, 256), N=# of elements
        ...
        # Linear->BN->LeakyReLU->Linear(out_dim=1)
        ...
        if memory_c is not None and features_c.shape[0] > 1 and memory_c.shape[0] > 1:
            ...
            features_c_norm = F.normalize(features_c, dim=1)  # (M pixels, 256)
            ...
            # now weight every sample by class specific attention module
            # selector: Linear -> BN -> LeakyReLU -> Linear
            learned_weights_features = selector(features_c.detach())  # (M pixels, 1) detach for trainability
            learned_weights_features_memory = selector_memory(memory_c)  # (N, 1)
            ...
            rescaled_weights = rescaled_weights.repeat(1, distances.shape[1])  # (M pixels, N)
            distances = distances * rescaled_weights

            learned_weights_features_memory = torch.sigmoid(learned_weights_features_memory)
            learned_weights_features_memory = learned_weights_features_memory.permute(1, 0)  # (1, N)
